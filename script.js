        ...
        return `
            <div class="client-card">
                ...
                <div class="client-stats">
                    ...
                    <div class="stat-item">
                        <span class="stat-label">Saldo Totale</span>
                        <span class="stat-value amount">€ ${saldoTotale.toFixed(2).replace('.', ',')}</span>
                    </div>
                </div>
    ...
    static renderConto(conto, clickable = true) {
        ...
        return `
            <div class="account-card" ${clickHandler}>
                ...
                <div class="account-balance">€ ${conto.saldo.toFixed(2).replace('.', ',')}</div>
            </div>
        `;
    }
    ...
    static renderPrestito(prestito) {
        ...
        return `
            <div class="account-card prestito-card">
                ...
                <div class="account-titolari">Importo: € ${prestito.importo.toFixed(2).replace('.', ',')}</div>
                <div class="account-balance">Rata: € ${prestito.rata.toFixed(2).replace('.', ',')}</div>
        ...
        return `
            <div class="movimento-item">
                ...
                <div class="movimento-amount" style="${colore}">
                    ${simbolo} € ${mov.importo.toFixed(2).replace('.', ',')}
                </div>
            </div>
        `;
    }
...
class StatsRenderer {
    static getDashboardStats() {
        ...
        return { clienti, conti, prestiti, saldoTotale, prestitiPending };
    }
        ...
        return `
            <div class="stats-container">
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-content">
                        <span class="stat-label">Saldo Totale</span>
                        <span class="stat-number amount">€ ${stats.saldoTotale.toFixed(2).replace('.', ',')}</span>
                    </div>
                </div>
                <div class="stat-card">
                    <div class="stat-icon">📋</div>
                    <div class="stat-content">
                        <span class="stat-label">Prestiti Pending</span>
...
function aggiornaModalitaApp() {
    ...
    if (searchInput) {
        // Dashboard search removed; keep hidden to avoid confusion
    ...
    const ricercaNavBtn = document.getElementById('ricercaNavBtn');
    if (ricercaNavBtn) {
        ricercaNavBtn.style.display = sessionMode === 'staff' ? 'inline-block' : 'none';
    }
...
function mostraSezione(sezione) {
    ...
    if (['gestioneAccount', 'ricerca'].includes(sezione) && sessionMode !== 'staff') {
    ...
    if (sezione === 'dashboard') {
        ...
        document.getElementById('ricercaSection').classList.remove('hidden');
        mostraRicerca();
    }
    ...
    try {
        ...
        if (contoCliente) {
            const movimento = {
                ...
                importo: 0,
                causale: `Richiesta prestito ${nuovoPrestito.id} - € ${importo.toFixed(2).replace('.', ',')}`,
    ...
    prestiti.forEach(prestito => {
        ...
        const html = `
            <div class="account-card prestito-card">
                ...
                <div class="account-titolari">Importo: € ${prestito.importo.toFixed(2).replace('.', ',')} - Durata: ${prestito.durata} mesi</div>
                <div class="account-balance">Rata: € ${prestito.rata.toFixed(2).replace('.', ',')}</div>
...
function mostraRicerca() {
    const results = document.getElementById('ricercaResults');
    const queryInput = document.getElementById('searchQueryInput');
    if (queryInput) {
        queryInput.value = '';
    }
    results.innerHTML = '<div class="empty-state"><p>Inserisci nome o IBAN per cercare un conto o un prestito.</p></div>';
}

function cercaPerQuery() {
    ...
    const prestiti = cercaPrestitiPerNome(query);

    if (conti.length === 0 && prestiti.length === 0) {
        results.innerHTML = '<div class="empty-state"><p>Nessun conto o prestito trovato.</p></div>';
        return;
    }
    ...
    if (conti.length > 0) {
        const contiHtml = `
            <div style="margin-bottom: 40px;">
                <h3 style="color: var(--primary); margin-bottom: 20px; border-bottom: 2px solid var(--accent); padding-bottom: 10px;">💳 Conti Trovati (${conti.length})</h3>
                <div class="accounts-grid">
                    ${conti.map(conto => `
                        <div class="account-card" onclick="apriDettagliConto('${conto.id}')">
                            ...
                            <div class="account-iban">${conto.iban}</div>
                            <div class="account-titolari">${conto.titolariString || ''}</div>
                            <div class="account-balance">Saldo: € ${conto.saldo.toFixed(2).replace('.', ',')}</div>
                        </div>
                    `).join('')}
                </div>
            </div>
        `;
        results.insertAdjacentHTML('beforeend', contiHtml);
    }
    ...
    if (prestiti.length > 0) {
        const prestitiHtml = `
            <div>
                <h3 style="color: var(--primary); margin-bottom: 20px; border-bottom: 2px solid #f39c12; padding-bottom: 10px;">🏦 Prestiti Trovati (${prestiti.length})</h3>
                <div class="accounts-grid">
        ...
        `;
        results.insertAdjacentHTML('beforeend', prestitiHtml);
    }
}
...
function apriGestioneClienteModal(clienteId) {
    ...
    const saldoTotale = conti.reduce((sum, c) => sum + (c.saldo || 0), 0);
    document.getElementById('gestioneClienteSaldoTotale').textContent = `€ ${saldoTotale.toFixed(2).replace('.', ',')}`;
    ...
    // Apri modal
    document.getElementById('gestioneClienteModal').classList.remove('hidden');
}
...
function caricaGestioneAccountUtenti() {
    const clienti = ottieniClienti();
    const lista = document.getElementById('userAccountsList');
    const gestioneResults = document.getElementById('gestioneSearchResults');
    if (gestioneResults) gestioneResults.innerHTML = '';

    if (!clienti.length) {
        lista.innerHTML = '<div class="empty-state"><p>Nessun account cliente trovato.</p></div>';
        return;
    }

    lista.innerHTML = '';
    clienti.forEach(cliente => {
        const contiCliente = ottieni_conti().filter(c => c.clienteId === cliente.id);
        const html = `
            <div class="account-card">
                <div class="account-header">
                    <span class="account-icon">👤</span>
                    <span class="account-type">${cliente.nome} ${cliente.cognome}</span>
                </div>
                <div class="account-iban">${cliente.email}</div>
                <div class="account-titolari">Account creato: ${cliente.registratoIl}</div>
                <div class="account-balance">Conti associati: ${contiCliente.length}</div>
                <button class="btn-danger" onclick="eliminaCliente('${cliente.id}')">Elimina account</button>
            </div>
        `;
        lista.insertAdjacentHTML('beforeend', html);
    });
...
function cercaContiInGestione() {
    ...
    const results = document.getElementById('gestioneSearchResults');
    if (!results) return;

    if (!query) {
        results.innerHTML = '<div class="empty-state"><p>Inserisci IBAN o nominativo per cercare un conto.</p></div>';
        return;
    }
    ...
    if (conti.length === 0) {
        results.innerHTML = '<div class="empty-state"><p>Nessun conto trovato.</p></div>';
        return;
    }
...
function apriDettagliConto(contoId) {
    ...
    document.getElementById('modalSaldo').textContent = `€ ${contoSelezionato.saldo.toFixed(2).replace('.', ',')}`;
    ...
    const canoneText = contoSelezionato.tipoConto === 'Canone Fisso' ? 'EUR 15,00/mese' :
                       ...
                       `Tasso: ${contoSelezionato.tassoInteresse}% - Interessi accumulati: EUR ${contoSelezionato.interessiAccumulati.toFixed(2).replace('.', ',')}`;
    ...
    // Mostra modal
    document.getElementById('dettagliContoModal').classList.remove('hidden');
}
...
function eseguiVersamento() {
    ...
    alert(`✓ Versamento di EUR ${netto.toFixed(2).replace('.', ',')} confermato${commissione > 0 ? `\nCommissione: EUR ${commissione.toFixed(2).replace('.', ',')}` : ''}`);
...
function eseguiPrelevamento() {
    ...
    alert(`✓ Prelevamento di EUR ${importo.toFixed(2).replace('.', ',')} confermato${commissione > 0 ? `\nCommissione: EUR ${commissione.toFixed(2).replace('.', ',')}` : ''}`);
...
function eseguiBonificio() {
    ...
    alert(`✓ Bonifico di EUR ${importo.toFixed(2).replace('.', ',')} a ${iban} confermato`);
...
function eseguiGiroconto() {
    ...
    alert(`✓ Giroconto di EUR ${importo.toFixed(2).replace('.', ',')} confermato`);
